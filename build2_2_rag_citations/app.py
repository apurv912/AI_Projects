...
For EVERY factual claim you make, add a citation like [Chunk 6].
Only cite chunk numbers that exist in the provided chunks.
If the chunks do not contain enough information, say: "I don't have enough information in the document to answer that."

Output format:
- Answer (short, structured bullets)
- Sources used: [Chunk X], [Chunk Y]
...
"""
    ...
    return resp.choices[0].message.content.strip()
def extract_chunk_citations(text: str) -> set[int]:
    """Extract citations like [Chunk 6] from model output."""
    return {int(x) for x in re.findall(r"\[Chunk\s+(\d+)\]", text)}


st.set_page_config(page_title="Build 2 — RAG Q&A", layout="wide")
st.title("Build 2.2 — RAG Doc Q&A — Citations")
st.caption("Upgrade: answers include [Chunk X] citations + simple citation validation.")
...
if run:
    ...
    st.write(answer)
    retrieved_ids = {i for i, _, _ in results}
    cited_ids = extract_chunk_citations(answer)
    ...
    if invalid:
        st.error(
            f"Invalid citations found: {sorted(invalid)}. "
            f"The model cited chunks not in retrieved set {sorted(retrieved_ids)}."
        )
    elif cited_ids:
        st.success(f"Citations OK: {sorted(cited_ids)}")
    else:
        if "I don't have enough information" not in answer:
            st.warning("No [Chunk X] citations detected. Try clicking again / re-running.")
